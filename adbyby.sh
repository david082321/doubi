...
check_installed_status(){
	[[ ! -e ${adbyby_file} ]] && echo -e "${Error} ADbyby 沒有安裝，請檢查 !" && exit 1
}
check_pid(){
...
Download_adbyby(){
	...
	[[ ! -e "adbyby.tar.gz" ]] && echo -e "${Error} ADbyby 下載失敗 !" && exit 1
	tar -xzf adbyby.tar.gz && rm -rf adbyby.tar.gz
	[[ ! -e "${adbyby_file}" ]] && echo -e "${Error} ADbyby 解壓失敗 !" && exit 1
	cd bin
	chmod 777 adbyby
	wget -N --no-check-certificate "https://raw.githubusercontent.com/david082321/doubi/master/other/adhook.ini"
}
Service_adbyby(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/david082321/doubi/master/other/adbyby_centos -O /etc/init.d/adbyby; then
			echo -e "${Error} ADbyby服務 管理腳本下載失敗 !" && exit 1
		fi
		chmod +x /etc/init.d/adbyby
		chkconfig --add adbyby
		chkconfig adbyby on
	else
		if ! wget --no-check-certificate https://raw.githubusercontent.com/david082321/doubi/master/other/adbyby_debian -O /etc/init.d/adbyby; then
			echo -e "${Error} ADbyby服務 管理腳本下載失敗 !" && exit 1
		fi
		chmod +x /etc/init.d/adbyby
		update-rc.d -f adbyby defaults
	fi
	echo -e "${Info} ADbyby服務 管理腳本下載完成 !"
}
...
Install_adbyby(){
	[[ -e ${adbyby_file} ]] && echo -e "${Error} 檢測到 ADbyby 已安裝 !" && exit 1
	check_sys
...
Start_adbyby(){
	check_installed_status
	check_pid
	[[ ! -z ${PID} ]] && echo -e "${Error} ADbyby 正在執行，請檢查 !" && exit 1
	service adbyby start
}
Stop_adbyby(){
	check_installed_status
	check_pid
	[[ -z ${PID} ]] && echo -e "${Error} ADbyby 沒有執行，請檢查 !" && exit 1
	service adbyby stop
}
Restart_adbyby(){
	check_installed_status
...
echo && echo -e "請輸入一個數字來選擇選項
 ...
 ${Green_font_prefix}1.${Font_color_suffix} 安裝 ADbyby
 ${Green_font_prefix}2.${Font_color_suffix} 移除 ADbyby
————————————
 ${Green_font_prefix}3.${Font_color_suffix} 啟動 ADbyby
 ${Green_font_prefix}4.${Font_color_suffix} 停止 ADbyby
 ${Green_font_prefix}5.${Font_color_suffix} 重啟 ADbyby
————————————
...
echo
stty erase '^H' && read -p " 請輸入數字 [1-8]:" num
case "$num" in
	1)
	Install_adbyby
	;;
