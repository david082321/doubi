...
check_pid(){
	...
	[[ -z ${PID} ]] && echo -e "${Error} ShadowsocksR服務端沒有執行，請檢查 !" && exit 1
}
scan_port_centos(){
	...
	[[ -z ${port} ]] && echo -e "${Error} 沒有發現正在連結的埠 !" && exit 1
	[[ ${port_num} = 0 ]] && echo -e "${Error} 沒有發現正在連結的埠 !" && exit 1
}
scan_port_debian(){
...
scan_ip_centos(){
	...
	ip_num=`echo "${ip}" |wc -l`
	[[ -z ${ip} ]] && echo -e "${Error} 沒有發現正在連結的IP !" && exit 1
	[[ ${ip_num} = 0 ]] && echo -e "${Error} 沒有發現正在連結的IP !" && exit 1
}
scan_ip_debian(){
...
check_threshold_centos(){
	...
	do
		...
		[[ ${ip_num} -ge ${IP_threshold} ]] && echo -e " 埠: ${Red_font_prefix}${port_check}${Font_color_suffix} ,IP總數: ${Red_font_prefix}${ip_num}${Font_color_suffix} ,IP: ${Sky_blue_font_prefix}$(echo "${ip_check}")${Font_color_suffix}"
		ip_check=""
	done
}
check_threshold_debian(){
	[[ ${action_2} == "y" ]] && echo -e "${Tip} 檢測IP歸屬地(ipip.net)，如果IP較多，可能時間會比較長..."
	for((integer = ${port_num}; integer >= 1; integer--))
	do
		port_check=`echo "${port}" |sed -n "$integer"p`
...
c_ssr(){
	...
	if [[ ${release} == "centos" ]]; then
		scan_port_centos
		echo -e "目前時間：${Yellow_font_prefix}$(date "+%Y-%m-%d %H:%M:%S %u %Z")${Font_color_suffix}\n"
		check_threshold_centos
	else
		scan_port_debian
		echo -e "目前時間：${Yellow_font_prefix}$(date "+%Y-%m-%d %H:%M:%S %u %Z")${Font_color_suffix}\n"
...
a_ssr(){
	...
	if [[ ${release} == "centos" ]]; then
		scan_port_centos
		scan_ip_centos
		echo -e "目前時間：${Yellow_font_prefix}$(date "+%Y-%m-%d %H:%M:%S %u %Z")${Font_color_suffix} ,目前連結的埠共 ${Red_font_prefix}${port_num}${Font_color_suffix} ,目前連結的IP共 ${Red_font_prefix}${ip_num}${Font_color_suffix} \n"
		check_threshold_centos
	else
		scan_port_debian
		scan_ip_debian
...
case "$action" in
 ...
 c 檢查並顯示 超過IP閾值的埠
 a 顯示目前 所有埠IP連接訊息
 y 顯示IP歸屬地(這是第二個參數如：bash ssr_ip_check.sh a y)"
    ;;
esac
