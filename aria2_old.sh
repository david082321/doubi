...
Installation_dependency(){
	if [[ ${release} = "centos" ]]; then
		...
		if [[ $? = 1 ]]; then
			echo -e "${Error} CentOS6 系統不支援安裝 Aria2（源沒有Aria2包），請更換 CentOS7 或其他系統安裝 !" && exit 1
		fi
		wget -N --no-check-certificate http://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm
		[[ ! -e "epel-release-latest-7.noarch.rpm" ]] && echo -e "${Error} CentOS7 EPEL 源下載失敗 !" && exit 1
		...
		wget --no-check-certificate -O "/etc/apt/sources.list" "https://raw.githubusercontent.com/david082321/doubi/master/sources/us.sources_stretch.list"
		apt-get update
	...
	[[ ! -e ${aria2c} ]] && echo -e "${Error} Aria2 安裝失敗，請檢查 !" && exit 1
}
Install_aria2(){
...
Read_config(){
	[[ ! -e ${aria2_conf} ]] && echo -e "${Error} Aria2 設定檔案不存在 !" && exit 1
...
————————————
 ...
 ${Green_font_prefix}7.${Font_color_suffix} 查看 日誌訊息
————————————" && echo
if [[ -e ${aria2c} ]]; then
	check_pid
	if [[ ! -z "${PID}" ]]; then
