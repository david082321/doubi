...
SSR_installation_status(){
	[[ ! -e ${config_user_file} ]] && echo -e "${Error} 沒有發現 ShadowsocksR 設定檔案，請檢查 !" && exit 1
...
View_User(){
	...
	if [[ -z "${now_mode}" ]]; then
		ss_ssr_determine
		clear && echo "===================================================" && echo
		echo -e " ShadowsocksR帳號 設定訊息：" && echo
		...
		echo -e " 埠總限速 : ${Green_font_prefix}${speed_limit_per_user} KB/S${Font_color_suffix}"
		echo -e "${ss_link}"
		echo -e "${ssr_link}"
 ...
 協議和混淆後面的[ _compatible ]，指的是 相容原版協議/混淆。"
		...
		[[ ${user_total} = "0" ]] && echo -e "${Error} 沒有發現 多埠使用者，請檢查 !" && exit 1
		clear && echo "===================================================" && echo
		echo -e " ShadowsocksR帳號 設定訊息：" && echo
		echo -e " I  P\t    : ${Green_font_prefix}${ip}${Font_color_suffix}"
		echo -e " 加密\t    : ${Green_font_prefix}${method}${Font_color_suffix}"
		...
		echo -e " 設備數限制 : ${Green_font_prefix}${protocol_param}${Font_color_suffix}"
		echo -e " 單執行緒限速 : ${Green_font_prefix}${speed_limit_per_con} KB/S${Font_color_suffix}"
		echo -e " 埠總限速 : ${Green_font_prefix}${speed_limit_per_user} KB/S${Font_color_suffix}" && echo
...
Set_config_port(){
	while true
	do
	echo -e "請輸入要設置的ShadowsocksR帳號 埠"
...
Set_config_protocol_param(){
	while true
	do
	echo -e "請輸入要設置的ShadowsocksR帳號 欲限制的設備數 (${Green_font_prefix} auth_* 系列協議 不相容原版才有效 ${Font_color_suffix})"
...
Set_config_speed_limit_per_con(){
	while true
	do
	echo -e "請輸入要設置的每個埠 單執行緒 限速上限(單位：KB/S)"
...
Set_config_speed_limit_per_user(){
	...
	do
	echo
	echo -e "請輸入要設置的每個埠 總速度 限速上限(單位：KB/S)"
	...
	if [[ $? == 0 ]]; then
		if [[ ${ssr_speed_limit_per_user} -ge 1 ]] && [[ ${ssr_speed_limit_per_user} -le 131072 ]]; then
			echo && echo ${Separator_1} && echo -e "	埠總限速 : ${Green_font_prefix}${ssr_speed_limit_per_user} KB/S${Font_color_suffix}" && echo ${Separator_1} && echo
			break
		else
			echo -e "${Error} 請輸入正確的數位(1-131072)"
		fi
...
Download_SSR(){
	...
	mkdir ${config_folder}
	[[ ! -e ${config_folder} ]] && echo -e "${Error} ShadowsocksR設定檔案的資料夾 建立失敗 !" && exit 1
	echo -e "${Info} ShadowsocksR服務端 下載完成 !"
}
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/david082321/doubi/master/other/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/david082321/doubi/master/other/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服務 管理腳本下載失敗 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
Install_SSR(){
	check_root
	[[ -e ${config_user_file} ]] && echo -e "${Error} ShadowsocksR 設定檔案已存在，請檢查( 如安裝失敗或者存在舊版本，請先移除 ) !" && exit 1
	...
	JQ_install
	echo -e "${Info} 開始寫入 ShadowsocksR設定檔案..."
	Write_configuration
...
debian_View_user_connection_info(){
	...
	if [[ -z "${now_mode}" ]]; then
		...
		user_list_all="埠: ${Green_font_prefix}"${user_port}"${Font_color_suffix}\t 連結IP總數: ${Green_font_prefix}"${user_IP_total}"${Font_color_suffix}\t 目前連結IP: ${Green_font_prefix}${user_IP}${Font_color_suffix}\n"
		user_IP=""
		echo -e "目前模式: ${Green_background_prefix} "${now_mode}" ${Font_color_suffix} 連結IP總數: ${Green_background_prefix} "${IP_total}" ${Font_color_suffix}"
		echo -e "${user_list_all}"
	else
		now_mode="多埠" && user_total=`${jq_file} '.port_password' ${config_user_file} |sed '$d;1d' | wc -l`
		IP_total=`netstat -anp |grep 'ESTABLISHED' |grep 'python' |grep 'tcp6' |awk '{print $5}' |awk -F ":" '{print $1}' |sort -u |grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}" |wc -l`
		user_list_all=""
		for((integer = ${user_total}; integer >= 1; integer--))
		do
			...
			user_list_all=${user_list_all}"埠: ${Green_font_prefix}"${user_port}"${Font_color_suffix}\t 連結IP總數: ${Green_font_prefix}"${user_IP_total}"${Font_color_suffix}\t 目前連結IP: ${Green_font_prefix}${user_IP}${Font_color_suffix}\n"
			user_IP=""
		done
		echo -e "目前模式: ${Green_background_prefix} "${now_mode}" ${Font_color_suffix} 使用者總數: ${Green_background_prefix} "${user_total}" ${Font_color_suffix} 連結IP總數: ${Green_background_prefix} "${IP_total}" ${Font_color_suffix} "
		echo -e "${user_list_all}"
	fi
}
...
Modify_Config(){
	SSR_installation_status
	if [[ -z "${now_mode}" ]]; then
		echo && echo -e "目前模式: 單埠，你要做什麼？
 ${Green_font_prefix}1.${Font_color_suffix} 修改 使用者埠
 ${Green_font_prefix}2.${Font_color_suffix} 修改 使用者密碼
 ...
 ${Green_font_prefix}7.${Font_color_suffix} 修改 單執行緒限速
 ${Green_font_prefix}8.${Font_color_suffix} 修改 埠總限速
 ${Green_font_prefix}9.${Font_color_suffix} 修改 全部設定" && echo
...
——————————
 ...
 ${Green_font_prefix}9.${Font_color_suffix}  修改 埠總限速
 ${Green_font_prefix}10.${Font_color_suffix} 修改 全部設定" && echo
		stty erase '^H' && read -p "(預設: 取消):" ssr_modify
		[[ -z "${ssr_modify}" ]] && echo "已取消..." && exit 1
		Get_User
		if [[ ${ssr_modify} == "1" ]]; then
...
List_multi_port_user(){
	...
	do
		...
		user_list_all=${user_list_all}"埠: "${user_port}" 密碼: "${user_password}"\n"
	done
	echo && echo -e "使用者總數 ${Green_font_prefix}"${user_total}"${Font_color_suffix}"
	echo -e ${user_list_all}
}
# 添加 多埠使用者設定
Add_multi_port_user(){
	...
	Add_iptables
	Save_iptables
	echo -e "${Info} 多埠使用者添加完成 ${Green_font_prefix}[埠: ${ssr_port} , 密碼: ${ssr_password}]${Font_color_suffix} "
}
# 修改 多埠使用者設定
Modify_multi_port_user(){
	List_multi_port_user
	echo && echo -e "請輸入要修改的使用者埠"
	...
	if [[ ! -z "${del_user}" ]]; then
		...
		Add_iptables
		Save_iptables
		echo -e "${Inof} 多埠使用者修改完成 ${Green_font_prefix}[舊: ${modify_user_port}  ${password} , 新: ${ssr_port}  ${ssr_password}]${Font_color_suffix} "
	else
		echo -e "${Error} 請輸入正確的埠 !" && exit 1
	fi
}
# 刪除 多埠使用者設定
Del_multi_port_user(){
	List_multi_port_user
	user_total=`${jq_file} '.port_password' ${config_user_file} | sed '$d' | sed "1d" | wc -l`
	[[ "${user_total}" = "1" ]] && echo -e "${Error} 多埠使用者僅剩 1個，不能刪除 !" && exit 1
	echo -e "請輸入要刪除的使用者埠"
	...
	if [[ ! -z ${del_user} ]]; then
		...
		echo -e "${Info} 多埠使用者刪除完成 ${Green_font_prefix} ${del_user_port} ${Font_color_suffix} "
	else
		echo "${Error} 請輸入正確的埠 !" && exit 1
	fi
}
# 手動修改 使用者設定
...
Port_mode_switching(){
	SSR_installation_status
	if [[ -z "${now_mode}" ]]; then
		echo && echo -e "	目前模式: ${Green_font_prefix}單埠${Font_color_suffix}" && echo
		echo -e "確定要切換為 多埠模式？[y/N]"
		...
		echo && echo -e "	目前模式: ${Green_font_prefix}多埠${Font_color_suffix}" && echo
		echo -e "確定要切換為 單埠模式？[y/N]"
		stty erase '^H' && read -p "(預設: n):" mode_yn
		[[ -z ${mode_yn} ]] && mode_yn="n"
		if [[ ${mode_yn} == [Yy] ]]; then
			user_total=`${jq_file} '.port_password' ${config_user_file} | sed '$d' | sed "1d" | wc -l`
...
Other_functions(){
  ...
  注意： 銳速/LotServer/BBR 不支援 OpenVZ！
  注意： 銳速/LotServer/BBR 不能共存！
————————————
...
Update_Shell(){
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/david082321/doubi/master/ssr.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		if [[ ${yn} == [Yy] ]]; then
			if [[ $sh_new_type == "softs" ]]; then
				...
				wget -N --no-check-certificate https://raw.githubusercontent.com/david082321/doubi/master/ssr.sh && chmod +x ssr.sh
			fi
...
menu_status(){
	if [[ -e ${config_user_file} ]]; then
		...
		now_mode=$(cat "${config_user_file}"|grep '"port_password"')
		if [[ -z "${now_mode}" ]]; then
			echo -e " 目前模式: ${Green_font_prefix}單埠${Font_color_suffix}"
		else
			echo -e " 目前模式: ${Green_font_prefix}多埠${Font_color_suffix}"
		fi
	else
...
————————————
  ...
  ${Green_font_prefix}9.${Font_color_suffix} 切換 埠模式
————————————
