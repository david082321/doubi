...
Service_caddy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/david082321/doubi/master/other/caddy_centos -O /etc/init.d/caddy; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/david082321/doubi/master/other/caddy_debian -O /etc/init.d/caddy; then
			echo -e "${Error_font_prefix}[錯誤]${Font_suffix} Caddy服務 管理腳本下載失敗 !" && exit 1
		fi
		chmod +x /etc/init.d/caddy
